...
#ifdef SERIAL_BARE_METAL

#define SERIAL_WRITABLE (UCSR0A & BV(UDRE0))
#define SERIAL_WRITE(byte) UDR0 = (byte);

#define SERIAL_AVAILABLE (UCSR0A & _BV(RXC0))
#define SERIAL_READ (uint8_t) UDR0

#else

#define SERIAL_WRITABLE Serial.availableForWrite()
#define SERIAL_WRITE(byte) Serial.write((byte));

#define SERIAL_AVAILABLE Serial.available()
#define SERIAL_READ (uint8_t) Serial.read()
...
void loop(void) {
  if (usart_rx_rdy()) {
    ...
    if (rx_state == 3) {
      ...
      if (rx_message[0] == EXIT_PROGMODE) {
        exit_progmode();
        usart_tx_b(0x82);
        rx_state = 0;
      }
      if (rx_message[0] == ISP_RESET_POINTER) {
        isp_reset_pointer();
        usart_tx_b(0x83);
        rx_state = 0;
      }
      if (rx_message[0] == ISP_SEND_CONFIG) {
        isp_send_config(0);
        usart_tx_b(0x84);
        rx_state = 0;
      }
      if (rx_message[0] == ISP_INC_POINTER) {
          for(i = 0; i < rx_message[2]; i++) { isp_inc_pointer(); }
        usart_tx_b(0x85);
        rx_state = 0;
      }
      if (rx_message[0] == ISP_READ_PGM) {
        usart_tx_b(0x86);
      ...
      if (rx_message[0] == ISP_MASS_ERASE) {
        isp_mass_erase();
        usart_tx_b(0x87);
        rx_state = 0;
      }

      if (rx_message[0] == ISP_WRITE_PGM) {
        for (i = 0; i < rx_message[2] / 2; i++) {
          flash_buffer[i] =
                (((unsigned int)(rx_message[(2 * i) + 1 + 4])) << 8) + (((unsigned int)(rx_message[(2 * i) + 0 + 4])) << 0);
        }
        isp_write_pgm(flash_buffer, rx_message[2] / 2, rx_message[3]);
        usart_tx_b(0x88);
        rx_state = 0;
      }
      if (rx_message[0] == ISP_RESET_POINTER_16D) {
        isp_reset_pointer_16d();
        usart_tx_b(0x89);
        rx_state = 0;
      }
      if (rx_message[0] == P18_ENTER_PROGMODE) {
        p18_enter_progmode();
        usart_tx_b(0x90);
        rx_state = 0;
      }
      if (rx_message[0] == P18_ISP_READ_PGM) {
        usart_tx_b(0x91);
          addr = (((unsigned long)(rx_message[3])) << 16) + (((unsigned long)(rx_message[4])) << 8) +
               (((unsigned long)(rx_message[5])) << 0);
      ...
      if (rx_message[0] == P18_ISP_WRITE_PGM) {
          addr = (((unsigned long)(rx_message[3])) << 16) + (((unsigned long)(rx_message[4])) << 8) +
               (((unsigned long)(rx_message[5])) << 0);
        for (i = 0; i < rx_message[2] / 2; i++) {
          flash_buffer[i] =
                (((unsigned int)(rx_message[(2 * i) + 1 + 6])) << 8) + (((unsigned int)(rx_message[(2 * i) + 0 + 6])) << 0);
        }
        p18_isp_write_pgm(flash_buffer, addr, rx_message[2] / 2);
        usart_tx_b(0x92);
        rx_state = 0;
      }
      if (rx_message[0] == P18_ISP_MASS_ERASE) {
        p18_isp_mass_erase();
        usart_tx_b(0x93);
        rx_state = 0;
      }
      if (rx_message[0] == P18_ISP_WRITE_CFG) {
          addr = (((unsigned long)(rx_message[3])) << 16) + (((unsigned long)(rx_message[4])) << 8) +
               (((unsigned long)(rx_message[5])) << 0);
      ...
      if (rx_message[0] == P18FJ_ISP_MASS_ERASE) {
        p18fj_isp_mass_erase();
        usart_tx_b(0xA3);
        rx_state = 0;
      }
      if (rx_message[0] == P18FK_ISP_MASS_ERASE) {
        p18fk_isp_mass_erase(rx_message[2], rx_message[3], rx_message[4]);
        usart_tx_b(0xB0);
        rx_state = 0;
      }
      if (rx_message[0] == P18FK_ISP_WRITE_PGM) {
          addr = (((unsigned long)(rx_message[3])) << 16) + (((unsigned long)(rx_message[4])) << 8) +
               (((unsigned long)(rx_message[5])) << 0);
        for (i = 0; i < rx_message[2] / 2; i++) {
          flash_buffer[i] =
      ...
      if (rx_message[0] == P18FK_ISP_WRITE_CFG) {
          addr = (((unsigned long)(rx_message[3])) << 16) + (((unsigned long)(rx_message[4])) << 8) +
               (((unsigned long)(rx_message[5])) << 0);
      ...
      if (rx_message[0] == P16C_ENTER_PROGMODE) {
        p16c_enter_progmode();
        usart_tx_b(0xC0);
        rx_state = 0;
      }
      if (rx_message[0] == P16C_ISP_READ_PGM) {
        usart_tx_b(0xC1);
          addr = (((unsigned long)(rx_message[3])) << 16) + (((unsigned long)(rx_message[4])) << 8) +
               (((unsigned long)(rx_message[5])) << 0);
      ...
      if (rx_message[0] == P16C_ISP_WRITE_PGM) {
          addr = (((unsigned long)(rx_message[3])) << 16) + (((unsigned long)(rx_message[4])) << 8) +
               (((unsigned long)(rx_message[5])) << 0);
        for (i = 0; i < rx_message[2] / 2; i++) {
          flash_buffer[i] =
      ...
      if (rx_message[0] == P16C_BULK_ERASE) {
        p16c_set_pc(0x8000);
        p16c_bulk_erase();
        usart_tx_b(0xC3);
        rx_state = 0;
      }
      if (rx_message[0] == P16C_ISP_WRITE_CFG) {
          addr = (((unsigned long)(rx_message[3])) << 16) + (((unsigned long)(rx_message[4])) << 8) +
               (((unsigned long)(rx_message[5])) << 0);
      ...
      if (rx_message[0] == P18Q_ISP_WRITE_CFG) {
          addr = (((unsigned long)(rx_message[3])) << 16) + (((unsigned long)(rx_message[4])) << 8) +
               (((unsigned long)(rx_message[5])) << 0);
        cfg_val = rx_message[6];
        cfg_val = (cfg_val << 8) + rx_message[7];
      ...
      if (rx_message[0] == P18Q_ISP_WRITE_PGM) {
          addr = (((unsigned long)(rx_message[3])) << 16) + (((unsigned long)(rx_message[4])) << 8) +
               (((unsigned long)(rx_message[5])) << 0);
        for (i = 0; i < rx_message[2] / 2; i++) {
          flash_buffer[i] =
...
unsigned char
rx_state_machine(unsigned char state, unsigned char rx_char) {
  if (state == 0) {
    rx_message_ptr = 0;
...
void
isp_read_pgm(unsigned int* data, unsigned char n) {
  unsigned char i;
...
void
isp_write_pgm(unsigned int* data, unsigned char n, unsigned char slow) {
  unsigned char i;
  //_delay_us(3*ISP_CLK_DELAY);
  for (i = 0; i < n; i++) {
    isp_send(0x02, 6);
...
void
isp_send_config(unsigned int data) {
  isp_send(0x00, 6);
  isp_send(data, 16);
}
...
void
isp_reset_pointer(void) {
  //_delay_us(3*ISP_CLK_DELAY);
  isp_send(0x16, 6);
}

void
isp_reset_pointer_16d(void) {
  //_delay_us(3*ISP_CLK_DELAY);
  isp_send(0x1D, 6);
  isp_send(0x0, 8);
  isp_send(0x0, 8);
...
void
isp_send(unsigned int data, unsigned char n) {
  unsigned char i;
  ISP_DAT_D_0
  //_delay_us(3*ISP_CLK_DELAY);
  for (i = 0; i < n; i++) {
...
void
isp_send_24_msb(unsigned long data) {
  unsigned char i;
  ISP_DAT_D_0
  //_delay_us(3*ISP_CLK_DELAY);
  for (i = 0; i < 23; i++) {
...
void
isp_send_8_msb(unsigned char data) {
  unsigned char i;
  ISP_DAT_D_0
  //_delay_us(3*ISP_CLK_DELAY);
  for (i = 0; i < 8; i++) {
...
void
p18fk_isp_mass_erase(unsigned char data1, unsigned char data2, unsigned char data3) {
...
void
p18fk_isp_write_pgm(unsigned int* data, unsigned long addr, unsigned char n) {
  unsigned char i;
  //_delay_us(3*ISP_CLK_DELAY);
  ...
  p18_set_tblptr(addr);
  for(i = 0; i < n - 1; i++) p18_send_cmd_payload(0x0D, data[i]);
  p18_send_cmd_payload(0x0F, data[n - 1]);
  p_18_modfied_nop(0);
}

void
p18_isp_write_pgm(unsigned int* data, unsigned long addr, unsigned char n) {
  unsigned char i;
  //_delay_us(3*ISP_CLK_DELAY);
...
void
p18_isp_write_cfg(unsigned char data1, unsigned char data2, unsigned long addr) {
  unsigned int i;
  //_delay_us(3*ISP_CLK_DELAY);
  p18_send_cmd_payload(0, 0x8EA6);
...
void
p18fk_isp_write_cfg(unsigned char data1, unsigned char data2, unsigned long addr) {
  unsigned int i;
  //_delay_us(3*ISP_CLK_DELAY);
  p18_send_cmd_payload(0, 0x8E7F);
...
void
p_18_modfied_nop(unsigned char nop_long) {
  unsigned char i;
  ISP_DAT_D_0
  ISP_DAT_0
  for (i = 0; i < 3; i++) {
...
void
p_18_isp_read_pgm(unsigned int* data, unsigned long addr, unsigned char n) {
  unsigned char i;
...
void
p18_set_tblptr(unsigned long val) {
...
void
p18_send_cmd_payload(unsigned char cmd, unsigned int payload) {
  isp_send(cmd, 4);
  isp_send(payload, 16);
  _delay_us(30);
}
...
unsigned int
p18_get_cmd_payload(unsigned char cmd) {
  isp_send(cmd, 4);
  return isp_read_16();
}
...
void
p16c_bulk_erase(void) {
  isp_send_8_msb(0x18);
  _delay_ms(100);
}

void
p16c_load_nvm(unsigned int data, unsigned char inc) {
  if (inc == 0)
    isp_send_8_msb(0x00);
  else
    isp_send_8_msb(0x02);
...
unsigned int
p16c_read_data_nvm(unsigned char inc) {
  unsigned int retval;
  unsigned char tmp;
  if (inc == 0)
    isp_send_8_msb(0xFC);
...
void
p16c_begin_prog(unsigned char cfg_bit) {
  isp_send_8_msb(0xE0);
  _delay_ms(3);
  if (cfg_bit != 0)
    _delay_ms(3);
...
void
p16c_isp_write_pgm(unsigned int* data, unsigned long addr, unsigned char n) {
  unsigned char i;
  //_delay_us(3*ISP_CLK_DELAY);
  p16c_set_pc(addr);
  for(i = 0; i < n; i++) p16c_load_nvm(data[i], 1);
  p16c_set_pc(addr);
  p16c_begin_prog(0);
}

void
p16c_isp_read_pgm(unsigned int* data, unsigned long addr, unsigned char n) {
  unsigned char i;
  unsigned int tmp1, tmp2;
  //_delay_us(3*ISP_CLK_DELAY);
  p16c_set_pc(addr);
  for(i = 0; i < n; i++) data[i] = p16c_read_data_nvm(1);
}

void
p16c_isp_write_cfg(unsigned int data, unsigned long addr) {
  unsigned char i;
  //_delay_us(3*ISP_CLK_DELAY);
  p16c_set_pc(addr);
...
void
p18q_isp_write_pgm(unsigned int* data, unsigned long addr, unsigned char n) {
  unsigned char i;
  //_delay_us(3*ISP_CLK_DELAY);
  p16c_set_pc(addr);
  for (i = 0; i < n; i++) {
...
void
p18q_isp_write_cfg(unsigned int data, unsigned long addr) {
  unsigned char i;
  //_delay_us(3*ISP_CLK_DELAY);
  p16c_set_pc(addr);
  isp_send_8_msb(0xE0);
...
uint8_t usart_rx_b(void) { return SERIAL_READ; }

void
usart_tx_hexa_8(uint8_t value) {
  uint8_t temp;
  temp = value;
  usart_tx_b('0');
  usart_tx_b('x');
...
void
usart_tx_hexa_8b(uint8_t value) {
  uint8_t temp;
  temp = value;
  temp = ((temp >> 4) & 0x0F);
  if (temp < 10)
...
void
usart_tx_hexa_16(uint16_t value) {
  usart_tx_b('0');
  usart_tx_b('x');
